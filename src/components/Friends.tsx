...
import { Search, UserPlus, UserCheck, Clock, Activity, Hash, Footprints } from 'lucide-react';
import { auth, db } from '../firebase';
import { collection, query, where, onSnapshot } from 'firebase/firestore';
import { UserProfile, Friendship, ExerciseLog } from '../types';
import { searchUsersByUsername, sendFriendRequest, acceptFriendRequest, getUserProfile } from '../utils/storage';
...
export const Friends: React.FC = () => {
  ...
  const groupedFriendLogs = friendLogs.reduce((acc, log) => {
    const date = new Date(log.date).toLocaleDateString('de-DE', { weekday: 'short', day: '2-digit', month: '2-digit' });
              ...
              {acceptedFriends.map(f => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all text-left ${isSelected ? 'bg-[#1d7a82]/20 border border-[#1d7a82]/50' : 'bg-black/20 border border-white/5 hover:bg-white/5'}`}
                  >
          ...
          {selectedFriend ? (
            <div className="bg-[#1e293b] rounded-2xl p-6 border border-white/10">
              <h2 className="text-lg font-semibold text-white mb-6 flex items-center gap-2">
                Aktivität von <span className="text-[#1d7a82] text-glow-teal">{friendsProfiles[selectedFriend]?.username}</span> (Letzte 7 Tage)
              </h2>
